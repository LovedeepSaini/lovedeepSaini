...
$.ajax({
    ...
    success: function(results) {
        ...
        for(let i=0; i < usersArray.length; i++){
            html_table += `<tr class="tableRow" id="${usersArray[i].id}"><td scope="row" class="tableIcon"style="display:none;"></td><td scope="row"  >${usersArray[i].firstName}</td><td scope="row" class="  d-none d-lg-table-cell ">${usersArray[i].lastName}</td><td scope="row" class="em d-none d-lg-table-cell " >${usersArray[i].email}</td><td scope="row" class="job d-none d-lg-table-cell ">${usersArray[i].jobTitle}</td><td scope="row" class="dep d-none d-lg-table-cell">${usersArray[i].department}</td><td scope="row" class="loc d-none d-lg-table-cell">${usersArray[i].location}</td><td scope="row" class="edit  "  ><button type="button"id="editbtn${usersArray[i].id}" class="btn btn-secondary"data-bs-toggle="modal" data-bs-target="#userSelectModal"><i class="fas fa-edit"></i></button> <button type="button"id="delete${usersArray[i].firstName}" class="btn btn-danger"data-bs-toggle="modal" data-bs-target="#userDeleteModal"><i class="fas fa-trash"></i></button></td></tr>`;
...
$('.depTableRow').click(function(){      
 ...
 $.ajax({
        ...
        success: function(results) {
            ...
            for(let i=0; i < data.length; i++){
                depArray.push(data[i]);
                $("#deptm").append(`<option value="${depArray[i].id}">${depArray[i].department}</option>`);
...
$.ajax({
    ...
    success: function(results) {
        ...
        for(let i=0; i < data.length; i++){
            locArray.push(data[i]);
            $("#loca").append(`<option value="${locArray[i].id}">${locArray[i].location}</option>`);
            function alphabetizeList() {
                var sel = $('#loca');
                var selected = sel.val();
                var opts_list = sel.find('option');
