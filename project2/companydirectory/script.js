...
$.ajax({
    ...
    success: function(results) {
        ...
        for(let i=0; i < usersArray.length; i++){
            html_table += `<tr class="tableRow" id="${usersArray[i].id}"><td scope="row" class="tableIcon">${usersArray[i].id}</td><td scope="row">${usersArray[i].firstName}</td><td scope="row">${usersArray[i].lastName}</td><td scope="row" class="em">${usersArray[i].email}</td><td scope="row" class="job">${usersArray[i].jobTitle}</td><td scope="row" class="dep">${usersArray[i].department}</td><td scope="row" class="loc">${usersArray[i].location}</td><td scope="row" class="edit"><button type="button"id="editbtn${usersArray[i].id}" class="btn btn-secondary"data-bs-toggle="modal" data-bs-target="#userSelectModal"><i class="fas fa-edit"></i></button> <button type="button"id="delete${usersArray[i].firstName}" class="btn btn-danger"data-bs-toggle="modal" data-bs-target="#userDeleteModal"><i class="fas fa-trash"></i></button></td></tr>`;
    ...
    $.ajax({
        ...
        success: function(results) {
            ...
            for(let i=0; i < locArray.length; i++){
                loc_html += `<tr id="${locArray[i].id}" class=" locationEdit locTableRow" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#locationEditModal" locationName="${locArray[i].location}" locationID="${locArray[i].id}" departments="${locArray[i].departments}"><td scope="row" class="locationHeader">${locArray[i].location}</td><td scope= "row"> <button type="update" id = "editloc"class="btn btn-secondary" data-bs-dismiss="modal" data-bs-target="locationEditModal">Edit </button>
                </td></tr>`;
...
function search() {
   ...
   let tr= mainTable.getElementsByTagName('tr');
   ...
   function searchdept() {
    // Sets each of the search sections values to a variable
   let filter= document.getElementById('searchdep').value.toUpperCase();
   let mainTable= document.getElementById('mainTable');
   let tr= mainTable.getElementsByTagName('tr');
   ...
   function searchlast() {
    // Sets each of the search sections values to a variable
   let filter= document.getElementById('searchl').value.toUpperCase();
   let mainTable= document.getElementById('mainTable');
   let tr= mainTable.getElementsByTagName('tr');
   ...
   function searchloca() {
    // Sets each of the search sections values to a variable
   let filter= document.getElementById('searchloc').value.toUpperCase();
   let mainTable= document.getElementById('mainTable');
   let tr= mainTable.getElementsByTagName('tr');
